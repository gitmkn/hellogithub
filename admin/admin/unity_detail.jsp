...
<DL class="infolist1" id="ilist4">
  ...
  <DIV class="tmore"><A 
  href="rlist.jsp?cs=18&cs3=<%=id%>&cs2=组建联盟联盟动态">更多&gt;&gt;</A></DIV></DT>
  <DD id="ilist4dd"> 
<%
						
						String sql1 = "select title,replace(title,substring(title from 15),'...'),date(createtime),id from dynamic where type=18 and state=1 and fatherID='"+id+"' order by createtime desc";
...
<DL class="infolist1" id="ilist5">
  ...
  <DIV class="tmore"><A 
  href="rlist.jsp?cs=19&cs3=<%=id%>&cs2=组建联盟新闻资讯">更多&gt;&gt;</A></DIV></DT>
  <DD>
 	  
	<%
						String sql3 = "select title,replace(title,substring(title from 15),'...'),date(createtime),id from dynamic where type=19 and state=1 and fatherID='"+id+"' order by createtime desc";
