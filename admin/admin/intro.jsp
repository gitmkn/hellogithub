...
<div class="lmenu">
     ...
     <dt class="title"><div class="tname">通知公告</div>
    <div class="tmore"><a href="rlist.jsp?cs=3&cs2=通知公告">更多>></a></div>
    </dt>
   <dd>
...
if(cs==null)cs="";
 String sql6 = "select pageview from dynamic where id='"+cs+"' and state=1";
						ResultSet rs6= mkn.executeQuery(sql6);
						rs6.last();
					    int x1 = rs6.getInt(1);
                  x1=x1+1;
                  String name = "";
                  String sql5="update dynamic set pageview='"+x1+"' where id='"+cs+"' and state=1";
                  boolean x2=mkn.executeUpdate(sql5,name);

						String sql = "select title,user,pageview,content from dynamic where id='"+cs+"'  and state=1 order by createtime desc limit 1";
						ResultSet rs2 = mkn.executeQuery(sql);
						rs2.last();
					    String t4 = rs2.getString(1);
						String t1 = rs2.getString(2);
						String t2 = rs2.getString(3);
						String t3= rs2.getString(4);
					%>
					<div class="tom">
<div class="totitle"><%=t4%><span></span>
</div></div>
      <div class="minfo"> 
         <h1 class="ititle"><%=t4%></h1>
        ...
        <%=t1%>
         ...
         <DIV class="container" style="word-wrap: break-word;overflow :hidden;background: #FFFFFF;margin-left: 15px;margin-right: 15px;height: auto;width: auto;">
         	<%=t3%>
         </DIV></DIV></DIV></DIV>
</BODY></HTML>
<%mkn.closeConnection(); %>
