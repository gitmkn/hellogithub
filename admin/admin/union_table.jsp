...
<a href="#">组建和参与联盟</a> &gt; 
<a href="#">参与联盟</a><span></span>
</div></div>
 
      <div class="content">
         <table border="1" cellspacing="0" cellpadding="0" width="600" align="center" 
         style="border-bottom: #cccccc 1px solid;border-left: #cccccc 1px dashed;border-top: #cccccc 1px;border-right: #cccccc 1px"> 
 <tbody> 
  <tr> 
   <td bgcolor="#e2e2e2" height="35" colspan="2" align="middle" 
   style="border-bottom: #cccccc 1px;border-left: #cccccc 1px;border-top: #cccccc 1px solid;
   border-right: #cccccc 1px dashed">参与联盟</td>
  </tr> 
  <%!int zys=1;
int gs=6;
int zgs=1;
...
if(ys<1)ys=1;
if(ys>zys)ys=zys;
String sql="select id,name from member where type=1 order by id desc";
...
String bt=rs1.getString(2);
if(x1%2==1){
%>
  <tr> 
   <td  class="table1" ><a href="unity_detail1.jsp?cs=<%=id%>" style="margin-left: 10px;color: #393c3d;" ><%=bt%></a></td> 
   <%}else{%>
   <td  class="table1"><a href="unity_detail1.jsp?cs=<%=id%>" style="margin-left: 10px;color: #393c3d;"><%=bt%></a></td> 
   <%
}
