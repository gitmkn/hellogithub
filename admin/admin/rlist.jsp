...
<BODY>
	...
	int ys = 1;%>
					<%
						...
						if (ys > zys)
							ys = zys;
						String sql2 = "select title,date(createtime),id from dynamic where type='"+cs+"' and state=1 and fatherID='"+cs3+"' order by createtime desc";
					...
					<h3 align=center>
						<a
							href="rlist.jsp?cs1=<%=ys - 1%>&cs=<%=cs%>&cs2=<%=cs2%>">
							...
							<%
								out.print(ys);
							%>
						 <a href="rlist.jsp?cs1=<%=ys + 1%>&cs=<%=cs%>&cs2=<%=cs2%>">
							<button class="btn btn-default">
